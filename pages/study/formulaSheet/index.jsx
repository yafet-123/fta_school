import { FaFlask, FaFolderOpen, FaStar } from "react-icons/fa";
...
const reasons = [
  {
    ...
    title: "Find Formula Sheets",
    description: "Access formula sheets organized by subject.",
  },
  {
    id: 2,
    icon: <FaFolderOpen className="text-green-500 w-8 h-8" />,
    title: "Study Easily",
    description: "All your required formula sheets in one place.",
  },
  {
    id: 3,
    icon: <FaStar className="text-yellow-500 w-8 h-8" />,
    title: "Boost Understanding",
    description: "Use well-structured formulas to improve your learning.",
  },
];
    ...
    <React.Fragment>
      ...
      <div className="bg-gray-50 min-h-screen py-20">
        ...
        <section className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-32 text-center px-6">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Formula Sheets</h1>
          <p className="text-lg md:text-xl mb-8">
            Access formula sheets organized by subject.
          </p>
        </section>
            ...
            {subjects.length === 0 ? (
              ...
              <div className="py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                {subjects.map((subject) => (
                  <Link href={`/study/formulaSheet/subject/${subject.id}`} key={subject.id}>
                    <div className="group bg-gradient-to-b from-white to-gray-50 rounded-2xl shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300 cursor-pointer flex flex-col items-center text-center p-6">
                      {subject.svg ? (
                        ...
                        <div className="w-full h-48 bg-gradient-to-br from-blue-100 to-indigo-200 rounded-xl mb-4 flex items-center justify-center">
                          <FaFlask className="text-blue-500 w-16 h-16" />
                        </div>
                      )}
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        {subject.formulaCount} formula sheet{subject.formulaCount !== 1 ? "s" : ""}
                      </p>
...
export async function getServerSideProps() {
  try {
    ...
    const formatted = subjects.map((sub) => ({
      ...
      name: sub.name,
      svg: sub.svg || null,
      formulaCount: sub.FormulaSheet?.length || 0,
    }));

    return { props: { subjects: JSON.parse(JSON.stringify(formatted)) } };
  } catch (error) {
    console.error("Error loading formula sheets:", error);
    try {
      const subjects = await prisma.subject.findMany({
        orderBy: { id: "asc" },
