...
export function DeleteDefinition({ setDeleteModalOn, item }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/definition/delete/${item.id}`);
      router.reload();
    } catch (error) {
      console.error(error);
        ...
        <div className="bg-white py-10 px-6 lg:px-10 border-4 border-red-500 rounded-xl shadow-xl w-full max-w-md">
          ...
          <div className="flex justify-center space-x-4">
            <button
              ...
              className="rounded px-5 py-2.5 font-semibold text-white bg-red-500 hover:bg-red-600 transition"
            >
