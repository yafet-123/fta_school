...
export function DisplayClass({classes}) {
    ...
    return (
        <div className="px-0 lg:px-10">
                            ...
                            {classes.map((data,index)=>(
                                <tr key={index} className="even:bg-neutral-300 odd:bg-neutral-200 even:dark:bg-gray-900 odd:dark:bg-gray-800 w-full">
                                    <td className="p-3 text-lg text-gray-700 whitespace-nowrap">
                                        <p className="font-bold text-gray-700 dark:text-white hover:underline">{data.class_id}</p>
                                    </td>
                                    ...
                                    <td className="p-3 text-lg text-gray-700 dark:text-white whitespace-nowrap">
                                        <button
                                            onClick={() => {
                                                ...
                                                setupdateclassname(data.ClassName)
                                            }} 
                                    ...
                                    <td className="p-3 text-lg text-gray-700 dark:text-white whitespace-nowrap">
                                        <button 
                                            onClick={() => {
                                                clickedFordelete()
                                                setdeleteclassid(data.class_id)
                                            }}
                        ...
                        <div key={index} className=" bg-neutral-200 dark:bg-slate-800 space-y-3 p-2 lg:p-4 rounded-lg shadow overflow-scroll">
                            <div>
                                <p className="text-blue-500 dark:text-white font-bold hover:underline">
                                    ...
                                    <span className="text-sm ">{data.class_id}</span>
                                </p>
                            </div>
            ...
            {deletemodalOn && 
                <DeleteClass setdeleteModalOn={setdeleteModalOn} deleteclassid={deleteclassid}/>
            }

            {updatemodalOn && 
                <UpdateClass setupdateModalOn={setupdateModalOn} updateclassid={updateclassid} updateclassname={updateclassname} setupdateclassname={setupdateclassname} setupdateclassid={setupdateclassid} />
            }
        </div>
    );
}
