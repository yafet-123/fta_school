...
export function DeleteBookCategory({ category, setDeleteModalOn }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/book-category/delete/${category.id}`);
      router.reload();
    } catch (error) {
      console.error(error);
      alert("Failed to delete Book Category. Please try again.");
      ...
      <div className="flex h-screen justify-center items-center px-4">
        <div className="bg-white py-10 px-6 lg:px-10 border-4 border-red-500 rounded-xl shadow-xl w-full max-w-md">
          ...
          <p className="text-center text-sm text-red-500 mb-6">
            ⚠️ This will also delete all books inside this category.
          </p>
          ...
          <div className="flex justify-center space-x-4">
            <button
              ...
              className={`rounded px-5 py-2.5 font-semibold ${
                loading
            ...
            <button
              ...
              className="rounded px-5 py-2.5 font-semibold text-white bg-gray-400 hover:bg-gray-500 transition"
            >
              Cancel
            </button>
          </div>
