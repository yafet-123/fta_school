...
export function UpdateSyllablus({ subjects, item, subjectId, setUpdateModalOn }) {
  ...
  const handleUpdate = async () => {
    if (!selectedSubjectId || !title || !content) {
      setError("Subject, Title, and Content Link are required.");
      return;
    }
    ...
    try {
      await axios.patch(`/api/syllablus/update/${item.id}`, {
      ...
      console.error(err);
      setError("Failed to update Syllabus. Try again.");
    ...
    <div className="bg-gray-200 bg-opacity-95 fixed inset-0 z-50">
      <div className="flex h-screen justify-center items-center px-4">
        <div className="bg-white py-10 px-8 lg:px-10 border-t-4 border-purple-500 rounded-2xl shadow-2xl w-full max-w-lg transition-all">
          ...
          <div className="mb-5">
            <label className="block mb-2 text-gray-700 font-semibold">Content Link</label>
            <input
              type="text"
              value={content}
          ...
          <div className="flex justify-center space-x-4">
            <button
              ...
              className="rounded-xl px-6 py-2.5 font-semibold text-white bg-purple-500 hover:bg-purple-600 transition-all"
            >
