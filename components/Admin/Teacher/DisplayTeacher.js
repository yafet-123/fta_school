...
export function DisplayTeacher({teachers}) {
    ...
    return (
        <div className="px-0 lg:px-10">
                            ...
                            {teachers.map((data,index)=>(
                                <tr key={index} className="even:bg-neutral-300 odd:bg-neutral-200 even:dark:bg-gray-900 odd:dark:bg-gray-800 w-full">
                                    <td className="p-3 text-lg text-gray-700 whitespace-nowrap">
                                        <p className="font-bold text-gray-700 dark:text-white hover:underline">{data.teacher_id}</p>
                                    </td>
                                    ...
                                    <td className="p-3 text-lg text-gray-700 dark:text-white whitespace-nowrap">
                                        <button 
                                            onClick={() => {
                                                clickedFordelete()
                                                setdeleteuserid(data.teacher_id)
                                            }}
            ...
            {deletemodalOn && 
                <DeleteTeacher setdeleteModalOn={setdeleteModalOn} deleteuserid={deleteuserid}/>
            }

            {updatemodalOn && 
                <UpdateTeacher setupdateModalOn={setupdateModalOn} updateuserid={updateuserid} updateemail={updateemail} updateusername={updateusername} setupdateemail={setupdateemail} setupdateusername={setupdateusername} />
            }
        </div>
    );
}
