...
export function AddQuestionCategory() {
    ...
    async function register(e){
        ...
        const data = await axios.post(`../../api/QuestionCategory/Add`,{
            ...
            console.log(error)
            seterror("Creating Question Category failed due to username is still exist or network error")
            setLoadingModalIsOpen(false);
        });
    }
    ...
    return (
        <div className="px-0 lg:px-10 pt-20">
            <form className="max-w-7xl mx-auto mt-10" onSubmit={register} >
                <h1 className="text-black dark:text-white text-xl lg:text-4xl font-bold text-center italic my-5">Question Category</h1>
